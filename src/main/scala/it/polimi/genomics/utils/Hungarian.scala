...
// use Java since `ArrayDeque` is faster than Scala's `Queue`
...
/** The `Hungarian` is an O(n&#94;3) implementation of the Hungarian algorithm
  ...
  */
class Hungarian(m: Int, n: Int, var cost: Array[Array[Double]]) {
  private val DEBUG = true
  // debug flag
  private val NA = -1
  // Not Assigned
  private val NO = -2 // None Possible
  //private val m :Int    = cost.dim1                       // m workers (x-nodes)
  //private val n  :Int   = cost.dim2                       // n jobs (y-nodes)

  private val r_m = 0 until m
  // range for workers
  private val r_n = 0 until n
  // range for jobs
  private val lx = Array.ofDim[Double](m)
  // labels of x-nodes (workers)
  private val ly = Array.ofDim[Double](n)
  // labels of y-nodes (jobs)
  private val slack = Array.ofDim[Double](n)
  // slack(y) = lx(x) + lx(y) - cost(x)( y)
  private val slackX = Array.ofDim[Int](n)
  // slackX(y) = x-node for computing slack(y)
  val xy = Array.fill(m)(NA)
  // xy(x) = y-node matched with x
  val yx = Array.fill(n)(NA)
  // yx(y) = x-node matched with y
  private val qu = new ArrayDeque[Int](m)
  // queue for Breadth First Search (BFS)
  private val maxMatch = min(n, m)
  // maximum number of matches needed
  ...
  private def initLabels() {
    if (DEBUG) {
      ...
      //      println("cost = " + cost)
      println("------------INPUT------------------------")
      println(cost.map(_.map(_.formatted("%3.3f")).mkString(", ")).mkString("\n"))
      //      for (x <- 0 to m - 1; y <- 0 to n - 1) {
      //        println("cost (" + x + ", " + y + ") = " + cost(x)(y))
      //      } // for
      println("------------INPUT------------------------")
    }
  ...
  private def findRootSetSlack(prev: Array[Int], xSet: Array[Boolean]) {
    ...
    for (y <- r_n) {
      ...
      slackX(y) = root
      if (DEBUG)
        println("augment: nMatch = " + nMatch + " need " + maxMatch + " --- " + "findRootSetSlack: slack(" + y + ") = " + slack(y) + ", slackX(" + y + ") = " + slackX(y))
    } // for
  } // findRootSetSlack
  ...
  private def augment() {
    ...
    val xSet = Array.fill(m) {
      false
    }
    // initialize xSet to empty
    val ySet = Array.fill(n) {
      false
    }
    // initialize ySet to empty
    val prev = Array.fill(m) {
      NA
    }
    // initialize prev to NA (for alternating tree)
    var edge = (NA, NA)
    // edge for augmenting path
        ...
        while (!qu.isEmpty()) {
          ...
          for (y <- r_n if cost(x)(y) == lx(x) + ly(y) && !ySet(y)) {
            if (yx(y) == NA) {
              edge = (x, y)
              break
            } // exposed x-node => augmenting path exists
  ...
  def solve(): Double = {
    if (m > n) {
      if (DEBUG)
        println("Hungarian: error - m = " + m + " > n = " + n)
      return -1.0
    }
...
object Hungarian {
  ...
  def apply(m: Int, n: Int, xy: Map[(Int, Int), Double]): Hungarian = {
    val c = Array.ofDim[Double](m, n) //new MatrixD (m, n)
    for ((k, v) <- xy) c(k._1)(k._2) = v
    new Hungarian(m, n, c)
  } // apply

  def apply(cost: Array[Array[Double]]): Hungarian = {
    val m: Int = cost.size
    val n: Int = cost(0).size
    new Hungarian(m, n, cost)
  } // apply
...
object HungarianTest extends App {
  ...
  //    5, 7, 6,
  //    5, 8, 8)
  val cost1: Array[Array[Double]] = Array(Array(1, 4, 5), Array(5, 7, 6), Array(5, 8, 8))
  ...
  println("optimal cost1 = " + (new Hungarian(3, 3, cost1).solve()))
  ...
  val costArif: Array[Array[Double]] = (Array(Array(0.9, 1.0), Array(0.8, 0.83), Array(1.0, 1.0)))
  println("-" * 100)


  val h = Hungarian(costArif)


  println("optimal costArif = " + h.solve())
  println("optimal xy costArif = " + h.xy.toList.mkString(" "))
  println("optimal yx costArif = " + h.yx.toList.mkString(" "))

  println("-" * 100)
  val h1 = Hungarian(costArif.transpose)


  println("optimal costArif = " + h1.solve())
  println("optimal xy costArif = " + h1.xy.toList.mkString(" "))
  println("optimal yx costArif = " + h1.yx.toList.mkString(" "))
